    ...
    for Xaxis in range(column):
        for Yaxis in range(column):
            if Table.checkbyzero(Xaxis, Yaxis) == 0:
                # set Ai One
                cellScore = StraightScan(Xaxis, Yaxis, 0, 3, 0, Table, CurrentPlayer, column) + OnceScan(Xaxis, Yaxis,
                                                                                                         0, 3, 2, Table,
                                                                                                         CurrentPlayer,
                                                                                                         column)
...
# Argument :
# column      default:20          type:int     ----- [int] (necessary) set the column of the gobang checkerboard(the size)
...
chess = ChessApp.start(column=20, inp1=AiOne, inp2=inputer, p1Name="a1", p2Name="AiOne", autoPrint=False)
